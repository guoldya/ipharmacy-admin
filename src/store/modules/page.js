...
import { getParentRouterObjByName, hasChild } from '@/utils/misc'
  ...
  actions: {
    ...
    openedLoad ({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        ...
        state.opened = value.map(opened => {
          // 忽略首页
          if (opened.fullPath === setting.page.opened[0].fullPath) {
            valid.push(1)
            return opened
          }
    ...
    open ({ state, commit, dispatch }, { name, params, query, fullPath, meta }) {
      return new Promise(async resolve => {
        ...
        state.curRoute = { name, params, query, fullPath, meta };
    ...
    closeAll ({ state, commit, dispatch }, vm) {
      ...
      return new Promise(async resolve => {
        ...
        if (vm.$route.name !== setting.page.opened[0].fullPath) {
          vm.$router.push({
            name: setting.page.opened[0].name
          })
        }
        // end
        resolve()
