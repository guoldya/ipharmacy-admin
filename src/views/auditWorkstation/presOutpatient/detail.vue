  ...
  <div class="detailPres">
      ...
      <a-col :span="14">
        <a-card>
          <a-row class="margin-top-10">
            <a-col class="titleText" :md="4" :lg="3" :xxl="3">{{leftData.patientDeptName}}：</a-col>
            <a-col :md="3" :lg="4" :xxl="4">
              <span class="font-bold fontSize14">{{leftData.patientName}}</span>
              &nbsp;&nbsp; {{leftData.patientSex |
            ...
            <a-col v-if="carePatient===true" :md="3" :lg="2" :xxl="3">
              <div class="guanzhu" :loading="loading">
                <a-icon theme="filled" type="star" class="xingxing"/>
                已关注
              </div>
            </a-col>
          </a-row>
    ...
    >
      <template slot="back">
        ...
        <a-button @click="slePatients(previousData)" :disabled="previousData.visId==0? true:false" class="margin-left-5"
                  :loading="loading">
          上一患者
        </a-button>
        <a-button @click="slePatients(nextPerson)" :disabled="nextPerson.visId==0? true:false" class="margin-left-5"
                  :loading="loading">
          下一患者
        </a-button>
      </template>
      <a-button @click="cancle" class="margin-left-5" :loading="loading">返回</a-button>
      <a-button @click="refuse" style="margin-left: 5px" :loading="loading" v-if="this.auditStatus===false">驳回
      </a-button>
      <a-button type="primary" class="margin-left-5" @click="submit" :loading="loading" v-if="this.auditStatus===false">
        通过
      </a-button>
    </footer-tool-bar>
  </div>
