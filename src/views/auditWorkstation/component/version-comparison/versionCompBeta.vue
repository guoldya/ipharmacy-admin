  ...
  <div>
    <a-row>
      ...
      <a-radio-group @change="onChange" v-model="reviewValue">
        <a-radio v-for="(dl,index) in orderList" :value="dl.reviewId" :key="index">{{dl.orderNo}}</a-radio>
      </a-radio-group>
    </a-row>
    <a-row class="margin-top-10">
      ...
      <a-radio-group v-model="submitValue" @change="changeEdition">
        <a-radio v-for="(dl,index) in reviewList" :value="dl.submitNo" :key="index">{{dl.edition}}</a-radio>
      </a-radio-group>
    </a-row>
    <a-row class="margin-top-10 versionTable">
      <a-col :span="12">
        <a-card title="当前版本" :extra="currentTime">
          <a-spin tip="加载中..." :spinning="currentSpinning">
            <el-table :data="currentData" :row-style="rowStyle" @row-click="rowClickCurrent">
              <el-table-column
                :prop="item.value"
                :label="item.title"
                :key="index"
                ...
                <template slot-scope="props">
                  <span v-if="item.value == 'drugName'" class="tableLineHeight">
                    ...
                    <div class="textIndentOne">
                      <div class="maxWidthTwo">
                        用法：{{props.row.dosageStr}}
                        <span class="padding5">{{props.row.useType}}</span>
                        {{props.row.frequency}}
                      </div>
                      <div
                        class="floatLeft"
                        v-if="props.row.auditLevelList.length>0"
      ...
      <a-col :span="12" class="padding-left-5">
        <a-card :title="submitEdition" :extra="otherTime">
          <a-spin tip="加载中..." :spinning="spinning">
            <el-table :data="otherData" :row-style="rowStyle" @row-click="rowClickOther">
              <el-table-column
                :prop="item.value"
                :label="item.title"
                :key="index"
...
export default {
  ...
  data() {
    return {
      ...
      spinning: false,
      columns: [
        { title: '#', value: 'seqNum', width: 30, align: 'left' },
  ...
  methods: {
    ...
    changeEdition(e) {
      ...
      console.log(this.otherData,'111')
    },
