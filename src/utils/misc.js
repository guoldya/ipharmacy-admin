...
/**
 * @param data  要处理数据，字符串
 * @param format 要处理的格式
 */
export function changeTime(data, format) {
  switch (format) {
    case 'day':
      data = data.replace(/(\d{2}:\d{2}:\d{2})$/g, '') //不要时分秒
      break
    case 'min':
      data = data.replace(/:\d{2}$/g, '') // 不要秒保留到分
      break
    case 'delyear':
      data = data.replace(/^\d{4}$/g, '') // 不要年
      break
    case 'delyearss':
      data = data.replace(/^(\d{4}-)|(:\d{2})$/g, '') // 不要年和秒
      break
  }
}
