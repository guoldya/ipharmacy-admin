<template>
  <a-card>
    ...
    <a-spin tip="加载中..." :spinning="spinLoading">
      <el-table class="margin-top-10" :data="drugData" border :highlight-current-row="true">
        <el-table-column fixed="right" label="操作" :width="100" align="center" v-if="true">
          <template slot-scope="scope">
    ...
    >
      <a-form :form="form">
        ...
        >
          <a-input
            v-decorator="[ 'specName',{rules: [{ required: true, message: '请输入名称' },{max: 20,message:'输入名称过长'},{ message: '请勿输入空格', pattern: /^[^\s]*$/}]} ]"
          />
        </a-form-item>
        <a-form-item label="备注" :label-col="{ span: 4 }" :wrapper-col="{ span: 17 }">
          <a-textarea v-decorator="[ 'remark',{rules: [{max: 50,message:'输入备注过长'}]}  ]" />
        </a-form-item>
        <a-form-item label="状态" :label-col="{ span: 4 }" :wrapper-col="{ span: 17 }">
          <a-radio-group v-decorator="[ 'status',{initialValue: 0} ]">
            <a-radio :value="op.id" v-for="(op,index) in status" :key="index">{{op.text}}</a-radio>
          </a-radio-group>
        </a-form-item>
      </a-form>
    </a-modal>
    ...
    >
      <a-form :form="drugForm">
        <a-form-item label="选择药品" :label-col="{ span: 4 }" :wrapper-col="{ span: 17 }">
          <a-select
            showSearch
            allowClear
            mode="single"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      drugAllList: [],
      columns: [
        { title: '编码', value: 'id', width: 80, align: 'right' },
      ...
      columns2: [
        { title: '药品编码', value: 'drugCode', width: 80, align: 'right' },
      ...
      items2: [{ text: '删除', color: '#ff9900', showtip: true, tip: '确认删除吗？', click: this.deleteDrug }],
      Modal: {
        visible: false
      },
      addModal: {
